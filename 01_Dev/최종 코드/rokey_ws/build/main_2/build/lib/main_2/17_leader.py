...
from std_msgs.msg import String, Bool  # 🔥 Bool 메시지 임포트 추가
...
class LeaderNode(Node):
    def __init__(self):
        ...
        self.is_event_paused = False # 🔥 이벤트에 의한 일시 정지 상태 플래그
        ...
        self.create_subscription(
            PoseWithCovarianceStamped, '/follower/amcl_pose', self.follower_pose_callback, 10, 
            callback_group=self.pose_cb_group)

        # 🔥 이벤트 노드 신호 수신 구독자 설정 (namespace 적용)
        self.create_subscription(
            Bool, '/robot4/event_trigger', self.event_callback, 10,
    ...
    # =========================================================
    # 🔥 이벤트 콜백 함수: True 수신 시 정지, False 수신 시 재개
    # =========================================================
    def event_callback(self, msg):
        is_event_active = msg.data

        if is_event_active and not self.is_event_paused:
            self.get_logger().warn("🚨 [Event] 이벤트 발생! 로봇 이동을 일시 정지합니다.")
            self.is_event_paused = True
            
            if self.goal_active:
                self.navigator.cancelTask() # 진행 중인 Nav2 작업 강제 취소
            self.set_state("Event_Pause")
            
        elif not is_event_active and self.is_event_paused:
            self.get_logger().info("✅ [Event] 이벤트 해제! 기존 목적지로 이동을 재개합니다.")
            self.is_event_paused = False
            
            # 목적지를 향해 가던 중이었다면 재출발
    ...
    def monitor_nav_status(self):
        ...
        # 🔥 이벤트로 인해 정지된 상태라면 Follower 체크나 도착 확인 로직을 스킵
        ...
        if self.navigator.isTaskComplete():
            ...
            if result == TaskResult.SUCCEEDED:
                ...
                # 🔥 cancelTask()가 호출되었을 때, 이것이 의도된 일시정지(Follower 대기 or Event 대기)인지 확인
