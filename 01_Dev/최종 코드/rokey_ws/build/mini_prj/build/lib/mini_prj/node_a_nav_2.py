...
class NodeAController(Node):
    ...
    patrol 중 /robot8/confidence 가 일정 프레임 이상 높게 유지되면
    현재 navigation goal 을 안전하게 취소하고 Node C 에게 SetBool service request 를 보낸다.
    """
    ...
    def __init__(self):
        ...
        self.started = False
        self.node_c_requested = False
        self.cancel_requested = False

        # Subscriber
        self.sub_from_wc = self.create_subscription(
            Bool,
            '/webcam/car_detected',
    ...
    def request_node_c_start(self):
        if not self.node_c_client.wait_for_service(timeout_sec=2.0):
            self.get_logger().error('Node C service /robot3/node_c_enable 을 찾을 수 없음')
            return False
    ...
    def execute_mission(self):
        ...
        for i, pose in enumerate(goal_pose):
            self.navigator.info(f'[{i+1}/{len(goal_pose)}] waypoint 이동 중...')
            ...
            self.navigator.info(f'[{i+1}/{len(goal_pose)}] 도착 완료! 360도 회전 시작')
            self.navigator.spin(spin_dist=2.0 * math.pi)
            ...
            while not self.navigator.isTaskComplete():
                ...
                if self.cancel_requested:
                    ...
                    if cancel_ok:
                        ...
                        self.get_logger().warn('spin 취소가 불확실하지만 Node C 요청을 시도함')
                        self.request_node_c_start()
