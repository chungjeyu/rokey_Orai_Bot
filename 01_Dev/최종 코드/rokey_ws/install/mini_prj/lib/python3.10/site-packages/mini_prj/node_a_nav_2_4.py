...
# =====================================================
    ...
    def slow_spin_search(self):
        ...
        천천히 회전하면서 YOLO 탐색
        감지되면 True 반환
        정상 360도 완료면 False 반환
        """

        self.get_logger().info('천천히 360도 탐색 회전 시작')

        msg = Twist()
        msg.angular.z = 0.30   # 회전 속도
        ...
        while rclpy.ok() and time.time() < end_time:
            ...
            # NodeB 감지 시 즉시 정지 후 종료
            if self.cancel_requested:
                self.stop_robot()
                self.get_logger().info('YOLO 감지 → 회전 중단')
                return True
        ...
        # 정상 회전 종료
        self.stop_robot()
        self.get_logger().info('360도 회전 완료')
        return False
    ...
    def execute_mission(self):
        ...
        for i, pose in enumerate(goal_pose):
            ...
            detected = self.slow_spin_search()

            # 회전 중 감지되면 Node C로 제어 이관
            if detected:
                cancel_ok = self.cancel_current_navigation()

                self.get_logger().info(
                    'Node A patrol 종료, Node C 로 제어 이관'
                )
